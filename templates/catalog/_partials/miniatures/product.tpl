{block name='product_miniature_item'}
  <article class="card" data-id-product="{$product.id_product}" 
  data-id-product-attribute="{$product.id_product_attribute}" itemscope itemtype="http://schema.org/Product">
    ...
    <div class="card-body">
      {block name='product_name'}
        <h5 class="card-title" itemprop="name"><a href="{$product.url}">{$product.name}</a></h5>
      {/block}

      {block name='product_description_short'}
        <div class="card-text" itemprop="description" hidden>{$product.description_short nofilter}</div>
      {/block}
